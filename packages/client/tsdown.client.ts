...
// Inlined from the upstream monorepo so this preset carries no workspace
// import chain (packages/client/modules, packages/client/web, root scripts).
...
/** Validate an optional manifest field that must be a string array when present. */
 ...
 * Upstream also forwards every DSH_CLIENT_* variable; this plugin embeds no
 * official-build variables, so the empty `process.env` substitution is the
 * whole contract (an unset static property read stays `undefined` without
 * providing a browser `process` global).
 */
