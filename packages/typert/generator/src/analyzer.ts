...
class FaceAnalyzer {
  ...
  private isTypeMetaSymbol(node: ts.Node, name: string): boolean {
    ...
    // Standalone builds consume the protocol from node_modules rather than a
    // workspace project; recognize its declarations by package path.
    if (isExternalTypertProtocolFile(declarationFile)) return true
...
/**
 * Files of the published Typert protocol package consumed from node_modules.
 * The in-workspace protocol project is recognized through package registration
 * ({@link WorkspaceAnalyzer.registrationForFile}); this covers standalone
 * builds where the protocol is an installed npm dependency rather than a
 * workspace project, so its declarations belong to no registration.
 */
function isExternalTypertProtocolFile(fileName: string): boolean {
  return slash(realPath(fileName)).includes('/node_modules/@deepseek-ai/dsh-typert-protocol/')
}
